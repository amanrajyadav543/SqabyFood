...
import 'package:sqabyfood_sqaby/data/model/response/distance_model.dart';
...
import 'package:sqabyfood_sqaby/data/model/response/timeslote_model.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  List<SubmitReviewModel>? _submitReview;
  ...
  SubmitDeliveryReviewModel? _submitDeliveryReviewModel;

  List<OrderModel>? get runningOrderList => _runningOrderList;
  List<OrderModel>? get historyOrderList => _historyOrderList;
  List<OrderDetailsModel>? get orderDetails => _orderDetails;
  List<SubmitReviewModel>? get submitReview => _submitReview;
  ...
  SubmitDeliveryReviewModel? get submitdelivery => _submitDeliveryReviewModel;
...
update();
  ...
  Future<List<SubmitReviewModel>?> getSubmitReviews(String orderID) async {
    _submitReview = null;
    _isLoading = true;
    _showCancelled = false;

    Response response = await orderRepo.getItemReviewDetails(orderID);
    _isLoading = false;
    if (response.statusCode == 200) {
      _submitReview = [];
      response.body.forEach((submitReview) => _submitReview?.
      add(SubmitReviewModel.fromJson(submitReview)));
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> getDeliverySubmitReviews(String orderID) async {

    Response response = await orderRepo.getSubmitDeliveryReviews(orderID);
    if (response.statusCode == 200) {

      _submitDeliveryReviewModel = SubmitDeliveryReviewModel.fromJson(response.body);
      update();
    } else {
      ApiChecker.checkApi(response);
    }
  ...
  Future<void> placeOrder(
      PlaceOrderBody placeOrderBody, Function callback, double amount,double wallet) async {
    _isLoading = true;
    update();
   ...
   //  log("--------------------------------$amount");
    if (response.statusCode == 200) {
      ...
      callback(true, message, orderID, amount,wallet);
      print('-------- Order placed successfully $orderID ----------');
     
    } else {
      callback(false, response.statusText, '-1', amount,wallet);
    }
    update();
  }
