...
import 'package:sqabyfood_sqaby/view/screens/auth/widget/code_picker_widget.dart';
import 'package:sqabyfood_sqaby/view/screens/auth/widget/condition_check_box.dart';
import 'package:sqabyfood_sqaby/view/screens/auth/widget/guest_button.dart';
...
class _SignInScreenState extends State<SignInScreen> {
  ...
  void initState() {
    ...
    smsVerification = Get.find<SplashController>().configModel?.SmsVerification;
    whatsappVerification = Get.find<SplashController>().configModel?.WhatsappVerification;
                ...
                child: Container(
                  ...
                  //    SizedBox(height: 10),
                      ...
                      !authController.isLoading ? Row(children: [
                        smsVerification! ? Expanded(child: CustomButton(
                          buttonText: 'sms'.tr,
                          color: true,
                          transparent: true,
                          onPressed: authController.acceptTerms ? () => _sms(authController, _countryDialCode!) : null,
                        )) : SizedBox(),
                        (smsVerification! && whatsappVerification!) ? SizedBox(width: 50,) :   SizedBox(width: 0,),
                        whatsappVerification! ?  Expanded(child: CustomButton(
                          buttonText: 'whatsapp'.tr,
                          onPressed: authController.acceptTerms ? () => _whatsapp(authController, _countryDialCode!) : null,
                        )) : SizedBox(),
                      ]) : Center(child: CircularProgressIndicator()),
                      SizedBox(height: 30),
                      GuestButton(),
  ...
  void _login(AuthController authController, String countryDialCode) async {
    ...
    //      String? _token = status.message?.substring(1, status.message?.length);
          if(Get.find<SplashController>().configModel!.customerVerification! && int.parse(status.message![0])
           == 0) {
        //    List<int> _encoded = utf8.encode(_password);
      //      String? _data = base64Encode(_encoded);
            Get.toNamed(RouteHelper.getVerificationRoute(_numberWithCountryCode, RouteHelper.signUp,false));
          }else {
  ...
  void _whatsapp(AuthController authController, String countryDialCode) async {
    String _phone = _phoneController.text.trim();
    //  String _password = _passwordController.text.trim();
    String _numberWithCountryCode = countryDialCode+_phone;
    bool _isValid = GetPlatform.isWeb ? true : false;
    ...
    if (_phone.isEmpty) {
      ...
      authController.whatsapp(_numberWithCountryCode).then((status) async {
        if (status.isSuccess!) {

          //    String _token = status.message.substring(1, status.message.length);
          // if(Get.find<SplashController>().configModel.customerVerification && int.parse(status.message[0]) == 0) {
          Get.toNamed(RouteHelper.getVerificationRoute(_numberWithCountryCode, RouteHelper.signUp,true));
          // }else {
          //   Get.toNamed(RouteHelper.getAccessLocationRoute('sign-in'));
          // }
          showCustomSnackBar("OTP Send Successfully");
  ...
  void _sms(AuthController authController, String countryDialCode) async {
    String _phone = _phoneController.text.trim();
    //  String _password = _passwordController.text.trim();
    String _numberWithCountryCode = countryDialCode+_phone;
    bool _isValid = GetPlatform.isWeb ? true : false;
    ...
    if (_phone.isEmpty) {
      ...
      authController.sms(_numberWithCountryCode).then((status) async {
        if (status.isSuccess!) {
